// Dining Partner Admin Service for Firestore Operations

import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:ticpin_dining/constants/model.dart';

class DiningPartnerAdminService {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final FirebaseAuth _auth = FirebaseAuth.instance;

  // ==================== CREATE ADMIN ====================

  /// Create dining partner admin document after OTP login
  Future<void> createAdminDocument(String phoneNumber) async {
    final user = _auth.currentUser;
    if (user == null) throw Exception('No user logged in');

    final adminDoc = _firestore
    ...
    if (!docSnapshot.exists) {
      final adminData = {
        ...
        'status': 'pending',
        'permissions': {
          'canManageDining': true,
          'canViewBookings': true,
          'canCancelBookings': false,
          'canUpdateMenu': true,
          'canViewAnalytics': true,
          'canManageTimings': true,
        ...
        'stats': {
          ...
          'fullPaymentRevenue': 0.0, // Full table bookings
          'pendingPayouts': 0.0,
          'averageRating': 0.0,
          'totalReviews': 0,
        },
      ...
      print('Dining partner admin document created for ${user.uid}');
    } else {
      print('Dining partner admin document already exists');
    }
  }

  // ==================== GET ADMIN ====================

  Future<DiningPartnerAdminModel?> getAdminData() async {
    final user = _auth.currentUser;
    ...
    return DiningPartnerAdminModel.fromFirestore(doc);
  }

  Stream<DiningPartnerAdminModel?> getAdminDataStream() {
    final user = _auth.currentUser;
    ...
    return _firestore
        ...
        .map(
          (doc) =>
              doc.exists ? DiningPartnerAdminModel.fromFirestore(doc) : null,
        );
  }

  /// Get admin data by ID (for super admin use)
  Future<DiningPartnerAdminModel?> getAdminDataById(String adminId) async {
    final doc = await _firestore
        .collection('dining_partner_admins')
        .doc(adminId)
        .get();
  ...
  Future<void> updateBusinessInfo(BusinessInfo businessInfo) async {
    ...
    await _firestore.collection('dining_partner_admins').doc(user.uid).update({
  ...
  Future<void> uploadDocuments({
    ...
    if (fssaiLicenseUrl != null) {
      updates['documentVerification.fssaiLicenseUrl'] = fssaiLicenseUrl;
      updates['documentVerification.fssaiStatus'] = 'pending';
    }
  ...
  Future<void> verifyDocument({
    required String adminId,
    required String
    documentType, // 'aadhar', 'pan', 'gst', 'fssai', 'businessLicense'
  ...
  /// Add dining to admin's managed dinings
  Future<void> addDining(String diningId) async {
    final user = _auth.currentUser;
    if (user == null) throw Exception('No user logged in');
  ...
  /// Remove dining from admin's managed dinings
  Future<void> removeDining(String diningId) async {
    final user = _auth.currentUser;
    if (user == null) throw Exception('No user logged in');
  ...
  /// Get all dinings managed by this admin
  Future<List<Map<String, dynamic>>> getManagedDinings() async {
    final user = _auth.currentUser;
    if (user == null) return [];

    final adminData = await getAdminData();
    if (adminData == null || adminData.diningIds.isEmpty) return [];

    final dinings = await Future.wait(
      adminData.diningIds.map((diningId) async {
        final doc = await _firestore.collection('dining').doc(diningId).get();
        if (doc.exists) {
    ...
    return dinings.whereType<Map<String, dynamic>>().toList();
  }

  Stream<QuerySnapshot> getManagedDiningsStream() {
    final user = _auth.currentUser;
    ...
    return _firestore
        ...
        .asyncExpand((adminDoc) {
          ...
          final diningIds = List<String>.from(
            adminDoc.data()?['diningIds'] ?? [],
          );
          if (diningIds.isEmpty) return const Stream.empty();

          return _firestore
              .collection('dining')
              .where(FieldPath.documentId, whereIn: diningIds)
              .snapshots();
        });
  }
  ...
  /// Get all bookings for admin's dinings
  ...
  /// bookingType: 'advance' (30% payment) or 'full' (full table booking)
  ...
  Future<void> updatePermissions({
    ...
    await _firestore.collection('dining_partner_admins').doc(adminId).update({
  ...
  Future<bool> hasPermission(String permissionName) async {
    ...
    switch (permissionName) {
      case 'canManageDining':
        return permissions.canManageDining;
      case 'canViewBookings':
        return permissions.canViewBookings;
      case 'canCancelBookings':
        return permissions.canCancelBookings;
      case 'canUpdateMenu':
        return permissions.canUpdateMenu;
      case 'canViewAnalytics':
        return permissions.canViewAnalytics;
      case 'canManageTimings':
        return permissions.canManageTimings;
