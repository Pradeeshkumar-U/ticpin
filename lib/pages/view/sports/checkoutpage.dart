...
// class _TurfCheckoutPageState extends State<TurfCheckoutPage>
...
//         final docId =
//             '${widget.turfId}_${widget.selectedDate}_${widget.fieldSize}';
//         final slotsRef = _firestore.collection('turf_slots').doc(docId);
//         final slotsDoc = await transaction.get(slotsRef);

//         Map<String, dynamic> currentSlots = {};
//         if (slotsDoc.exists) {
//           currentSlots = Map<String, dynamic>.from(
//             slotsDoc.data()!['slots'] ?? {},
//           );
//         }
...
//         transaction.set(
//           _firestore.collection('turf_bookings').doc(newBookingId),
//           bookingData,
//         );
...
//                             SizedBox(height: size.safeHeight * 0.2),
...
//                             child: ElevatedButton(
//                               onPressed:
//                                   _timerExpired ? null : _proceedToPayment,
//                               style: ElevatedButton.styleFrom(
//                                 backgroundColor: blackColor,
//                                 padding: EdgeInsets.symmetric(vertical: 16),
//                                 shape: RoundedRectangleBorder(
...
class _TurfCheckoutPageState extends State<TurfCheckoutPage>
  ...
  Future<void> _loadExistingBookingTimer() async {
    if (bookingId == null) return;
  ...
  void _navigateToTurfPage() {
    // Pop all routes until we're back at turf page
    Navigator.of(
      context,
    ).popUntil((route) => route.isFirst || route.settings.name == 'TurfPage');
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        // Always go back to turf page, not booking page
        _navigateToTurfPage();
